...
# 이벤트 분석을 위한 새로운 Pydantic 모델
class ReportMetadata(BaseModel):
    """보고서 메타데이터를 나타내는 모델"""
    filename: str = Field(description="문서명")
    company: str = Field(description="기업명")
    institution: str = Field(description="작성기관")
    date: str = Field(description="보고서 작성일")
    analyst: Optional[str] = Field(description="작성 애널리스트", default=None)

class EventImpact(BaseModel):
    """이벤트의 영향도를 나타내는 모델"""
    expected_market_reaction: str = Field(description="이벤트 발생 시 시장에 미칠 주요 영향")
    expected_business_impact: str = Field(description="기업의 사업 운영, 실적 전망 등에 미칠 영향")
    expected_risk_factors: str = Field(description="이벤트 발생 시 예상되는 위험 요소")

class SubsequentEvent(BaseModel):
    """후속 이벤트를 나타내는 모델"""
    description: str = Field(description="이벤트로 인해 발생할 가능성이 높은 후속 이벤트")
...
class CoreEvent(BaseModel):
    ...
    event_impact: EventImpact = Field(description="이벤트의 영향도")
    subsequent_events: List[SubsequentEvent] = Field(description="후속 이벤트 목록")

class EventAnalysis(BaseModel):
    """이벤트 분석 결과를 나타내는 모델"""
    report_metadata: ReportMetadata = Field(description="보고서 메타데이터")
...
class RagflowAnalyzer:
    ...
    def _generate_event_schema_prompt(self, schema: Dict) -> str:
        """이벤트 분석을 위한 스키마 프롬프트를 생성합니다."""
        prompt_parts = []
    ...
    def analyze_events(self, doc_name: str) -> Dict:
        """보고서의 이벤트를 분석합니다."""
        doc_info = self.parse_filename(doc_name)
        print(f"=== 이벤트 분석 시작 ===")
        print(f"파일명: {doc_name}")
        ...
        if not chunks:
            print(f"문서 청크를 찾을 수 없음: {doc_name}")
            return {**doc_info, "error": "문서 청크를 찾을 수 없음"}
        
        content = "".join(chunks)
        print(f"=== 문서 청크 수: {len(chunks)} ===")
        
        assistant = self._get_or_create_chat_assistant()
        session = assistant.create_session(f"{doc_info['date']}-{doc_info['company']}-events")
        
        try:
            print("=== 이벤트 분석 챗봇 응답 처리 시작 ===")
            
            # 이벤트 분석을 위한 프롬프트 생성
            schema_prompt = self._generate_event_schema_prompt(self.event_schema)
            
            prompt_template = (
                "당신은 세계 최고의 금융 전문가이자 이벤트 분석 시스템 개발자입니다. "
                "증권사 애널리스트 보고서를 분석하여 기업별·시점별 핵심 이벤트를 체계적으로 수집 및 정리하세요.\n\n"
                "분석 시 다음 사항을 참고하세요:\n"
                "1. 단순한 수치 데이터나 의견이 아닌, 실질적인 변화와 사건에 초점을 맞추세요.\n"
                "2. 이벤트의 시장 및 기업 영향도를 반드시 포함하세요.\n"
                "3. 연쇄 반응 및 파생 이벤트의 가능성을 분석하세요.\n"
                "4. 보고서 원문을 가능한 한 그대로 인용하세요.\n"
                "5. 후속 이벤트에는 확률(0.0~1.0)을 부여하세요.\n\n"
                "분석할 때 다음 스키마 정보를 참고하세요:\n\n"
                f"{schema_prompt}\n\n"
                "문서 정보:\n"
                "- 문서명: {filename}\n"
            ...
            try:
                # JSON 응답을 파싱
                event_analysis = json.loads(result_content)
                
                # 결과 저장
                output_dir = "output/events"
                os.makedirs(output_dir, exist_ok=True)
                
                output_file = f"{output_dir}/{doc_info['company']}_{doc_info['date']}_events.json"
                with open(output_file, 'w', encoding='utf-8') as f:
                    json.dump(event_analysis, f, ensure_ascii=False, indent=2)
                
                print(f"=== 이벤트 분석 결과 저장 완료: {output_file} ===")
                return event_analysis
                
            except Exception as e:
                print(f"=== 이벤트 분석 파싱 오류 ===")
                print(f"오류 메시지: {str(e)}")
                return {
                    **doc_info,
                    "error": f"이벤트 분석 파싱 실패: {str(e)}",
                    "raw_content": result_content
                }
            
        except Exception as e:
            print(f"=== 이벤트 분석 중 오류 발생 ===")
            print(str(e))
            return {**doc_info, "error": str(e), "raw_content": result_content if 'result_content' in locals() else ""}
    ...
    def analyze_all_events(self) -> List[Dict]:
        ...
        results = []
        for doc in self.get_sorted_documents():
            result = self.analyze_events(doc['original_filename'])
            results.append(result)
        return results
...
def main():
    ...
    if TEST_MODE:
        ...
        # 보고서 분석
        print("\n=== 보고서 분석 시작 ===")
        report_result = analyzer.analyze_report(TEST_FILE)
        analyzer.save_to_text([report_result], "test_result.txt")
        analyzer.save_to_excel([report_result], "test_result.xlsx")
        
        print("\n=== 보고서 분석 결과 ===")
        print(f"회사명: {report_result['company']}")
        print(f"작성기관: {report_result['institution']}")
        print(f"작성일: {report_result['date']}")
        
        if 'error' in report_result:
            print("=== 보고서 분석 오류 발생 ===")
            print(report_result['error'])
            if 'raw_content' in report_result:
                print("원본 응답:")
                print(report_result['raw_content'])
        else:
            print("=== 상세 분석 ===")
        ...
        # 이벤트 분석
        print("\n=== 이벤트 분석 시작 ===")
        event_result = analyzer.analyze_events(TEST_FILE)
        
        print("\n=== 이벤트 분석 결과 ===")
        print(f"회사명: {event_result['report_metadata']['company']}")
        print(f"작성기관: {event_result['report_metadata']['institution']}")
        print(f"작성일: {event_result['report_metadata']['date']}")
        
        if 'error' in event_result:
            print("=== 이벤트 분석 오류 발생 ===")
            print(event_result['error'])
            if 'raw_content' in event_result:
                print("원본 응답:")
                print(event_result['raw_content'])
        else:
            print("=== 핵심 이벤트 목록 ===")
            for event in event_result['core_events']:
                print(f"\n이벤트 ID: {event['event_id']}")
                print(f"카테고리: {event['category']}")
                print(f"유형: {event['type']}")
                print(f"설명: {event['description']}")
                print(f"원문: {event['source_quote']}")
                print("영향도:")
                print(f"- 시장 반응: {event['event_impact']['expected_market_reaction']}")
                print(f"- 사업 영향: {event['event_impact']['expected_business_impact']}")
                print(f"- 위험 요소: {event['event_impact']['expected_risk_factors']}")
                print("후속 이벤트:")
                for sub_event in event['subsequent_events']:
                    print(f"- {sub_event['description']} (확률: {sub_event['probability']})")
    else:
        print("전체 보고서 분석 중...")
        
        # 보고서 분석
        print("\n=== 보고서 분석 시작 ===")
        report_results = analyzer.analyze_all_reports()
        analyzer.save_to_text(report_results, "output/all_results.txt")
        analyzer.save_to_excel(report_results, "output/all_results.xlsx")
        print(f"보고서 분석 완료: {len(report_results)}개 보고서 처리됨")
        
        # 이벤트 분석
        print("\n=== 이벤트 분석 시작 ===")
        event_results = analyzer.analyze_all_events()
        print(f"이벤트 분석 완료: {len(event_results)}개 보고서 처리됨")
        
        # 전체 결과 요약
        print("\n=== 분석 결과 요약 ===")
        print(f"총 처리된 보고서 수: {len(report_results)}")
        print(f"보고서 분석 결과: output/all_results.txt, output/all_results.xlsx")
        print(f"이벤트 분석 결과: output/events/ 디렉토리")
